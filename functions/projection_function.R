...
#   {
    ...
    forest_biome <- raster::raster(forest_biome_path)
    
    terra::values(forest_biome) <- ifelse(terra::values(forest_biome) > 0, 1, NA)
    
    
    non_forest_biome <- raster::raster(non_forest_biome_path)
    
    terra::values(non_forest_biome) <- ifelse(terra::values(non_forest_biome) > 0, 1, NA)
    
    
    
    overlap_cells <- which(terra::values(sum(forest_biome,non_forest_biome,na.rm = TRUE)) == 2)
    ...
    weighted_lu <-
      get_and_weight_lu_rasters(
        raster_dir = raster_dir,
        luh = luh,
        model_levels = land_use_levels,
        match_land_use = land_use_to_raster, intensity = c("primary forest", "primary non-forest",
    ...
    forest_constants <- data.frame(
      ...
      nat_hab_sw = scale_object(0, alpha_data$nat_hab_sw)
    ...
    terra::values(alpha_baseline_rast)[overlap_cells] <- apply(matrix(c(terra::values(non_forest_alpha_baseline_rast)[overlap_cells],
                                                                        terra::values(forest_alpha_baseline_rast)[overlap_cells]), ncol = 2),MARGIN = 1,
    ...
    FII_alpha_diversity <- terra::rast(FII_alpha_diversity)
    ...
    forest_similarity_relative_raster <- terra::rast(forest_similarity_relative_raster)
    ...
    non_forest_similarity_relative_raster <- terra::rast(non_forest_similarity_relative_raster)
    ...
    terra::values(similarity_relative_raster)[overlap_cells] <- apply(matrix(c(terra::values(forest_similarity_relative_raster)[overlap_cells],
                                                                               terra::values(non_forest_similarity_relative_raster)[overlap_cells]), ncol = 2),MARGIN = 1,
                                                                      FUN = function(x) mean(x))
