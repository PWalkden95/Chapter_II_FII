...
boot_plot <-
  ninety_five_boot %>% pivot_longer(cols = colnames(ninety_five_boot), names_to = "LUI")


boot_plot$LUI <-
  factor(boot_plot$LUI, levels = land_use_levels)


realm_size_plot <-
  ggplot(data = boot_plot,
         aes(x = LUI, y = value, group = LUI)) +
  geom_violin(
    aes(colour = LUI, fill = LUI),
    alpha = 0.5,
    show.legend = FALSE
  ) +
  geom_boxplot(width = 0.1,
               aes(fill = LUI),
               show.legend = FALSE) +
  geom_hline(yintercept = median(ninety_five_boot$`Primary forest_Minimal use`), linetype = "dashed") +
...
confint(model_3, method = "boot", nsim = 200, boot.type = "perc")


data <- alpha_data
land_use <- "Primary forest_Minimal use"
variable = "nat_hab_sw"
...
test <- booted_confidence_intervals(data = alpha_data, land_use = "Primary non-forest_Minimal use", variable = "log_hpd_1km")
