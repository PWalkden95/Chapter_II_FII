...
Tom_data <- readRDS("data/diversity_combined_sites_Aug2022.rds")

sp_replace <- read.csv("data/missing_species_tom.csv")

for(i in 1:nrow(sp_replace)){
  Tom_data[which(Tom_data$Birdlife_Nominate == sp_replace$missing_species[i]),"Birdlife_Nominate"] <-
    sp_replace$patrick_predict_swap[i]
}

Tom_data[which(Tom_data$Birdlife_Nominate == "Icterus_3" & Tom_data$SSB == "GN1_2007__VanBael 1 1"),"Birdlife_Nominate"] <-
  "Icterus_2"

my_predicts <- readRDS("data/refined_predicts.rds")

my_predicts <- dplyr::select(my_predicts,colnames(my_predicts)[c(22,29:87)]) %>% dplyr::distinct()
...
PREDICTS <-
  ...
  dplyr::rename(Birdlife_Name = Birdlife_Nominate) %>%
  dplyr::left_join(my_predicts)
...
PREDICTS[which(PREDICTS$Birdlife_Name == "Hypocnemis striata"), c(23:35)] <-
  traits_df[which(traits_df$uniqueIDS == "Hypocnemis striata"),-1]

PREDICTS[which(PREDICTS$Birdlife_Name == "Hypocnemis striata"), c(36:81)] <-
  Forage[which(Forage$Birdlife_Name == "Hypocnemis striata"),-1]

PREDICTS[which(PREDICTS$Birdlife_Name == "Poeoptera femoralis"), c(23:35)] <-
  traits_df[which(traits_df$uniqueIDS == "Poeoptera femoralis"),-1]

PREDICTS[which(PREDICTS$Birdlife_Name == "Poeoptera femoralis"), c(36:81)] <-
  Forage[which(Forage$Birdlife_Name == "Poeoptera femoralis"),-1]


  
write_rds("data/tom_refined_data.rds", x = PREDICTS)
...
# miss_sp <- PREDICTS_speices[which(!(PREDICTS_speices %in% unique(AVONET_full$Species1_BirdLife)))]
# 
# write.csv("data/missing_species_tom.csv", x = miss_sp, row.names = FALSE)
...
write_rds("outputs/traits_without_foraging_list_tom.rds", x = FII_trait_list)
